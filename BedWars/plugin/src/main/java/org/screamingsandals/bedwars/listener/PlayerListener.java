...
import net.milkbowl.vault.economy.Economy; // [经济系统]
import org.bukkit.*;
import org.bukkit.block.Block;
import org.bukkit.block.BlockFace;
import org.bukkit.block.BlockState;
import org.bukkit.block.data.Waterlogged;
import org.bukkit.configuration.ConfigurationSection; // [经济系统]
...
import org.bukkit.plugin.RegisteredServiceProvider; // [经济系统]
...
public class PlayerListener implements Listener {
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (Main.isPlayerInGame(victim)) {
            ...
            if (game.getStatus() == GameStatus.RUNNING && victimTeam != null) {
                ...
                if (gVictim != null && !gVictim.isSpectator) {
                    ...
                    if (econ != null) {
                        ...
                        if (victimBalance > 0) {
                            long quarter = (long) Math.floor(victimBalance / 4.0);
                            double deduct = 2 * quarter;
                            if (victimBalance < deduct) {
                                deduct = victimBalance;
                            }
                            econ.withdrawPlayer(victim, deduct);
                            victim.sendMessage(ChatColor.RED + "你损失了 " + deduct + " 货币");
                            syncLevel(victim);

                            if (killer != null && Main.isPlayerInGame(killer) && Main.getPlayerGameProfile(killer).getGame() == game) {
                                econ.depositPlayer(killer, quarter);
                                killer.sendMessage(ChatColor.GREEN + "你获得了 " + quarter + " 货币（来自击杀）");
                                syncLevel(killer);
                            }
                        }
                    }
    ...
    public static void onItemPickup(Player player, Item item, Cancellable cancel) {
        if (Main.isPlayerInGame(player)) {
            ...
            if (game.getStatus() == GameStatus.RUNNING && !gPlayer.isSpectator) {
                ...
                ConfigurationSection rates = Main.getConfigurator().config.getConfigurationSection("economy-rates");
                if (rates != null) {
                    double copperRate = rates.getDouble("copper", 50.0);
                    double ironRate = rates.getDouble("iron", 5.0);
                    double goldRate = rates.getDouble("gold", 10.0);
                    double emeraldRate = rates.getDouble("emerald", 200.0);

                    double value = 0;
                    if (type.name().equals("COPPER_INGOT")) {
                        value = copperRate * amount;
                    } else if (type == Material.IRON_INGOT) {
                        value = ironRate * amount;
                    } else if (type == Material.GOLD_INGOT) {
                        value = goldRate * amount;
                    } else if (type == Material.EMERALD) {
                        value = emeraldRate * amount;
                    }
                    ...
                    if (value > 0) {
                        ...
                        if (econ != null) {
                            ...
                            syncLevel(player);
                            player.sendMessage(ChatColor.GREEN + "+ " + value + " 货币");
                        }
                        return;
                    }
                }
